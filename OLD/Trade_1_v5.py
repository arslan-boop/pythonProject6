    ...
    with open('../Sembol.txt', 'r') as dosya:
        for line in dosya.read().splitlines():
            ...
            if v_change_1m>=0.5 and  v_change_5m>0.5 and  v_change_15m>0.5\
                ...
                print('Sembol (ALIM ) = ', v_symbol,'Son Fiyat = ',v_son_fiyat,'Değişim', str(v_change_1m),'Zaman=',datetime.now())
                #dosya.close()
...
def Coin_Sat(v_symboly, v_last_price, v_interval):
   ...
   if (v_ema_cross_down7 == True) or (v_son_fiyat > v_alis_k): #or (v_son_fiyat < v_alis_z) : # 1 dk lıkta EMA5 - EMA20 ters kesince veya %1 kar görünce sat
      ...
      v_satim = 0
      print('ALIM BEKLETİLİYOR --SATIŞ İÇİN ŞARTLAR OLUŞMADI !!! ',v_son_fiyat,'SEMBOL=',v_symboly  )
...
if __name__ == '__main__':
    ...
    while True:
        ...
        #time.sleep(2)
        try:
            if (v_first == 'DEN' and v_second =='DENE'): #İçerde alım yoksa
                # Uygun coini bul
                ...
                if v_bulunan !='XXX':
                    ...
                    v_alim_var = 1
            elif (v_first != 'DEN') or (v_second !='DENE'): # Biri boş diğeri dolu
                if (v_first == 'DEN'):  #1. boş
                    v_bulunan, v_last_price, v_inter = Coin_Bul()
                    v_first = v_bulunan
                    if v_bulunan !='XXX':
                        print('ILK ALIM YAPILACAK =', datetime.now(), 'Bulunan = ', v_bulunan, 'Last Price = ', v_last_price)
                    ...
                    if v_bulunan != 'XXX':
                        ...
                        Coin_Al(v_bulunan, v_last_price, v_inter)
                        v_alim_var = 2
            elif (v_first != 'DEN') and (v_second != 'DENE'):  # İkisi de dolu
            ...
            # Alınanların Satılması
            if v_alim_var ==1 and v_first !='DEN' and v_second =='DENE' : # 1.dolu 2.boş
                    ...
                    if v_satim ==1:
                       ...
                       v_alim_var = 0
                       v_first ='DEN'
            elif  v_alim_var ==1 and  and v_first !='DEN' and v_second =='DENE' : # 1.dolu 2.boş
                # print('Alımmm varrr mıı', str(v_alim_var))
                v_bulunan = v_first
                Coin_Sat(v_bulunan, v_last_price, v_inter)
                # ---------------------------------------------
