        ...
        for line in dosya.read().splitlines():
            ...
            if v_4h_olusmadi == 0:  # 4h oluştuysa hepsi tamamdır
                if v_change_1m >= v_degis_oran and v_change_3m >= v_degis_oran and v_change_5m > v_degis_oran and v_change_15m > v_degis_oran \
                    ...
                    v_return_coin = 'XXX'
                    v_interval = '1m'
            if v_4h_olusmadi == 1 and v_2h_olusmadi == 0:  # 4 saatlik oluşmamıştır. Buna bakma
                if v_change_1m >= v_degis_oran and v_change_3m >= v_degis_oran and v_change_5m > v_degis_oran and v_change_15m > v_degis_oran \
            ...
            if v_4h_olusmadi == 1 and v_2h_olusmadi == 1 and  v_1h_olusmadi == 0:  # 4h, 2h oluşmadı
            ...
            if v_4h_olusmadi == 1 and v_2h_olusmadi == 1 and v_1h_olusmadi == 1 and v_15m_olusmadi == 0:  # 4-2-1 oluşmadı
...
def Coin_Al(v_symboly, v_last_price, v_interval):
    ...
    # v_time_bef = v_time
    DB_transactions.Delete_Table(v_symboly, 'TIP-1')
...
while True:
    ...
    try:
        ...
        if v_alim_var == 0:
            print('İçerde alım yok !!!!  ', v_alim_var)
            # Uygun coini bul
            ...
            if v_bulunan != 'XXX':
                ...
                if v_dosya_gun_counter == 5:
                    print('Dosya Yenileniyor!!!!!!!!!!', datetime.now())
                    Telebot_v1.mainma('Dosya Yenileniyor!!!!!!!!!!' + '-' + str(datetime.now()))
                    DB_transactions.USDT_Tablo_Yaz()
                    DB_transactions.File_write()
                    v_dosya_gun_counter = 0
                v_alim_var = 0
        if v_alim_var == 1:
            print('Alım Yapılmış Satışı Bekleniyor = ', v_bulunan ,  str(v_alim_var))
            v_satim = Coin_Sat(v_bulunan, v_last_price, v_inter)
            # ---------------------------------------------
            if v_satim == 1:
                print(v_bulunan , ' Coin Satıldı. Yeniden coin bulunacak ', str(v_satim))
                v_alim_var = 0
    except Exception as exp:
        v_hata_mesaj = 'Hataa!! = '+ str(v_bulunan) + 'Hata Kodu = '+ str(exp)
        print('Hataa!! =' , v_hata_mesaj)
